...
- A **Warmup Reps stepper**, editable right here — earlier, the rep count could only be changed from Edit Exercise Details, a real gap when you're mid-workout and just want to bump it. Edit Mode is still the only place that ever writes to `progression[]`; changing warmup reps here doesn't create a progression entry, only working-set changes do (see "Does not touch" below in §10, and the verified invariant there).
